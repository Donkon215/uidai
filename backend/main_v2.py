...
if not data_manager.pincode_summary.empty:
    ...
    logger.info("ML models applied to data")
...
async def get_overview_stats():
    ...
    return {
        ...
        "sector_alerts": {
            "education": int((df['School_Dropout_Risk_Index'] > 70).sum()),
            "hunger": int((df['Migrant_Hunger_Score'] > 70).sum()),
            "rural": int((df['Village_Hollow_Out_Rate'] > 70).sum()),
            "electoral": int((df['Electoral_Discrepancy_Index'] > 70).sum()),
            "labor": int((df['Skill_Gap_Migration_Flow'] > 70).sum())
        },
        "ml_stats": {
